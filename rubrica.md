...
Evidência complementar: o monitor administrativo de plataformas sincroniza a oferta retornada pela API após salvar preço e disponibilidade, liberando no mesmo fluxo a inclusão de keys para uma plataforma recém-configurada. Lint, TypeScript e detector automático foram executados; a validação manual em aparelho permanece pendente.
...
| Diagrama entidade-relacionamento | 0,5 | 👤 | Modelo futuro em `documentacao/evolucao/der.dbml`, baseado nos models e migrations, resumido a 7 tabelas e 36 colunas de compras, pagamentos e reserva de chaves para apoiar a explicação dos indicadores. Link do diagrama no dbdiagram incorporado à proposta. Confirmação final do autor pendente. |
...
| 16/09/2026 | Campos de data editáveis do painel mobile (lançamento e período de ofertas) passaram a usar máscara brasileira `DD/MM/AAAA`, validar datas reais e converter para ISO apenas no envio à API. Lint executado; teste manual em aparelho permanece pendente. | IA |
...
| 16/09/2026 | Corrigidos dois fluxos administrativos mobile: a exclusão de plataformas ganhou confirmação e tratamento de erro, e o monitor de keys passou a recarregar pelo `listingId` devolvido pela API após o cadastro. | IA |
...
| 16/09/2026 | Navegação das abas mobile passou a aceitar arraste horizontal entre Início, Loja, Carrinho, Perfil e Admin autorizado, preservando rolagem vertical, carrosséis e uso do teclado. Lint, TypeScript, detector e diff verificados; teste manual em aparelho permanece pendente. | IA |
| 16/09/2026 | Mensagens genéricas da API no mobile passaram a indicar o próximo passo: respostas de validação orientam revisar o formulário, enquanto sessão, permissão, conexão e instabilidade já possuem retornos distintos. Lint executado; validação manual do cadastro de jogo permanece pendente. | IA |
| 16/09/2026 | Corrigido o fluxo de plataforma recém-configurada no painel mobile: a resposta do salvamento atualiza imediatamente a oferta selecionada e seu `listingId`, permitindo adicionar keys sem reabrir a tela. Lint, TypeScript e detector automático executados; validação manual em aparelho permanece pendente. | IA |
| 16/09/2026 | Cards de jogos do painel administrativo mobile passam a usar uma coluna em janelas compactas, preservando os rótulos inteiros de ações como “Plataformas”. Lint, detector e diff executados; validação manual em aparelho permanece pendente. | IA |
| 17/09/2026 | DER futuro ampliado com a tabela existente `game_keys`, pedido e prazo de reserva, além do vínculo da chave entregue em `order_items`. Proposta e HTML atualizados com o ciclo disponível, reservada e vendida; nenhuma alteração no banco executada. | IA |
