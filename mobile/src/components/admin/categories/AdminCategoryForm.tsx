...
export default function AdminCategoryForm({ id }: { id?: string }) {
  ...
  useEffect(() => {
    ...
    void api
      .get<CategoryResponse>(`/categories/${id}`)
  ...
  const submit = async () => {
    ...
    try {
      ...
      if (editing) {
        ...
        router.replace("/admin/categories" as never);
      } else {
        await api.post("/categories", { name: value });
        setName("");
        setSuccess(`Categoria criada: ${value}`);
      }
    } catch (requestError) {
      setError(
        getApiErrorMessage(
  ...
  return (
    <AdminLayout
      ...
      description="Formulário para manter as categorias da loja."
      backTo="/admin/categories"
    >
      {loading ? (
        <Text style={adminStyles.muted}>Carregando formulário...</Text>
      ) : (
        <View style={adminStyles.card}>
          ...
          <AdminFormActions
            ...
            onCancel={() => router.replace("/admin/categories" as never)}
            submitLabel="Salvar"
            saving={saving}
          />
        </View>
