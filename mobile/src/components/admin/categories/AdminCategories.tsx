...
export default function AdminCategories() {
  ...
  const load = useCallback(async () => {
    try {
      ...
      const data = await api.get<PaginatedResponse<Category>>(
        `/categories?page=${page}&limit=${PAGE_SIZE}`,
      );
  ...
  return (
    <AdminLayout
      title="Categorias"
      description="Mantenha a classificação usada na loja e no painel."
      backTo="/admin"
      actions={
        <AdminLinkButton to="/admin/categories/new">
          Nova categoria
        </AdminLinkButton>
      }
    >
      ...
      >
        <View style={adminStyles.card}>
          <Text style={adminStyles.muted}>
            {meta.total} categoria(s) cadastrada(s)
          </Text>
            ...
            <View key={item.id} style={styles.item}>
              ...
              <View style={adminStyles.wrap}>
                <AdminButton
                  tone="secondary"
                  onPress={() =>
                    router.push(`/admin/categories/${item.id}/edit` as never)
                  }
                >
                  Editar
                </AdminButton>
                ...
                <AdminButton
                  tone="subtleDanger"
                  disabled={deleting && pending?.id === item.id}
      ...
      <AdminConfirmModal
        ...
        message={`Tem certeza que deseja excluir a categoria ${pending?.name ?? ""}?`}
        processing={deleting}
        tone="danger"
