...
export function getGameKeyValues(text: string) { const keyValues = text.split(/\r?\n|[\s,;]+/).map(formatGameKeyValue).filter(Boolean); return { keyValues, hasIncompleteKey: keyValues.some((value) => value.replace(/[^A-Z0-9]/g, "").length !== 12) }; }
