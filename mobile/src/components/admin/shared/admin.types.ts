...
export type AdminOfferListing = {
  ...
  price: number;
  isActive: boolean;
  game: { id: number; title?: string | null; coverImageUrl?: string | null } | null;
  platform: { id: number; name?: string | null; slug?: string | null } | null;
  pricing: {
    basePrice: number;
...
export type AdminPriceHistoryItem = {
  ...
  nextPrice: number;
  createdAt?: string;
  game: { id: number; title?: string | null } | null;
  platform: { id: number; name?: string | null; slug?: string | null } | null;
  changedBy: { id: number; username?: string | null; email?: string | null } | null;
};

export type StockSummary = { available: number; reserved: number; sold: number; total: number };
export type PlatformMonitorItem = {
  platform: { id: number; name: string; slug: string; iconUrl?: string | null; isActive?: boolean };
  hasListing: boolean;
...
export type GameKey = { id: number; keyValue: string; status: string };
export type SaveKeysResponse = { listingId: number; createdCount: number; skippedCount: number; stock: StockSummary };
export type DeleteKeysResponse = { stock: StockSummary };
