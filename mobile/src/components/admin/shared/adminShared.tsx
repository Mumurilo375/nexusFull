...
import {
  ActivityIndicator, Keyboard, Modal, ScrollView, StyleSheet, useWindowDimensions, View } from "react-native";
...
export function AdminToggleField({ label, checked, onChange }: { label: string; checked: boolean; onChange: (checked: boolean) => void }) {
  return <Pressable accessibilityRole="switch" accessibilityState={{ checked }} onPress={() => onChange(!checked)} style={styles.toggleRow}><Text style={adminStyles.label}>{label}</Text><View style={[styles.toggle, checked && styles.toggleOn]}><MotionIndicator active={checked} variant="toggle" style={[styles.toggleKnob, checked && styles.toggleKnobOn]} /></View></Pressable>;
}
export function AdminNotice({ children, tone = "error" }: { children: ReactNode; tone?: "error" | "success" }) { return <MotionView motionKey={typeof children === "string" ? children : tone} accessibilityLiveRegion="polite" style={[styles.notice, tone === "success" ? styles.successNotice : styles.errorNotice]}><Ionicons name={tone === "success" ? "checkmark-circle-outline" : "alert-circle-outline"} size={20} color={tone === "success" ? "#6ee7b7" : "#fda4af"} /><Text style={tone === "success" ? styles.successText : styles.errorText}>{children}</Text></MotionView>; }
...
export function AdminPageState({ loading, error, isEmpty, loadingText, emptyText, onRetry, loadingContent, children }: { loading: boolean; error?: string; isEmpty?: boolean; loadingText: string; emptyText: string; onRetry?: () => void; loadingContent?: ReactNode; children: ReactNode }) {
  if (loading) return loadingContent ?? <View style={styles.state}><ActivityIndicator color={adminColors.cyan} /><Text style={adminStyles.muted}>{loadingText}</Text></View>;
  if (error) return <View style={styles.errorState}><AdminNotice>{error}</AdminNotice>{onRetry ? <AdminButton tone="secondary" onPress={onRetry}>Tentar novamente</AdminButton> : null}</View>;
...
export function AdminSuccessToast({ title, message, onDismiss }: { title: string; message: string; onDismiss: () => void }) {
  ...
  return <MotionView accessibilityLiveRegion="polite" style={styles.toast}><Text style={styles.toastTitle}>{title}</Text><Text style={styles.toastMessage}>{message}</Text></MotionView>;
}

const styles = StyleSheet.create({
  button: { minHeight: 48, alignItems: "center", justifyContent: "center", borderWidth: 1, borderRadius: 12, paddingHorizontal: 16 },
  buttonText: { fontSize: 13, lineHeight: 18, fontWeight: "700", textAlign: "center" },
  ...
  notice: { flexDirection: "row", alignItems: "flex-start", gap: 10, borderWidth: 1, borderRadius: 12, padding: 13 }, errorNotice: { borderColor: "rgba(244,63,94,0.42)", backgroundColor: adminColors.dangerSoft }, successNotice: { borderColor: "rgba(16,185,129,0.42)", backgroundColor: adminColors.successSoft }, errorText: { flex: 1, color: "#fecdd3", fontSize: 14, lineHeight: 20 }, successText: { flex: 1, color: "#a7f3d0", fontSize: 14, lineHeight: 20 }, badge: { alignSelf: "flex-start", borderWidth: 1, borderRadius: 999, paddingHorizontal: 10, paddingVertical: 5 }, activeBadge: { borderColor: "rgba(59,130,246,0.32)", backgroundColor: adminColors.primarySoft }, inactiveBadge: { borderColor: adminColors.border, backgroundColor: adminColors.deep }, eyebrow: { color: "#bfdbfe", fontSize: 11, fontWeight: "800", letterSpacing: 1.6, textTransform: "uppercase", marginBottom: 10 }, state: { minHeight: 130, alignItems: "center", justifyContent: "center", gap: 10 }, errorState: { gap: 12 }, empty: { borderWidth: 1, borderColor: adminColors.softBorder, borderRadius: 16, backgroundColor: adminColors.surface, padding: 18 }, pagination: { flexDirection: "row", alignItems: "center", justifyContent: "space-between", gap: 8 }, contentCompact: { paddingHorizontal: 16 }, panelCompact: { borderWidth: 0, borderRadius: 0, padding: 0 }, header: { flexDirection: "row", alignItems: "flex-start", gap: 12 }, headerCompact: { flexDirection: "column" }, actions: { flexShrink: 1, alignItems: "flex-end", gap: 8 }, actionsCompact: { width: "100%", alignItems: "stretch" }, navButton: { minHeight: 44, paddingHorizontal: 13 }, backButton: { alignSelf: "flex-start", minHeight: 44, paddingHorizontal: 12, flexDirection: "row", gap: 7 }, backText: { color: adminColors.secondary, fontSize: 13, fontWeight: "700" }, bottomNav: { minHeight: 68, overflow: "hidden", flexDirection: "row", alignItems: "center", justifyContent: "space-around", borderTopWidth: 1, borderTopColor: adminColors.softBorder, backgroundColor: adminColors.surface, paddingHorizontal: 6 }, bottomNavButton: { minHeight: 54, minWidth: 0, flex: 1, marginHorizontal: 3, flexDirection: "row", alignItems: "center", justifyContent: "center", gap: 5, borderRadius: 12 }, bottomNavButtonActive: { backgroundColor: adminColors.primary, paddingHorizontal: 6 }, bottomNavLabel: { flexShrink: 1, color: adminColors.white, fontSize: 11, fontWeight: "700" }, toast: { position: "absolute", top: 24, left: 20, right: 20, borderWidth: 1, borderColor: "rgba(16,185,129,0.45)", borderRadius: 16, backgroundColor: "#020617", padding: 16 }, toastTitle: { color: "#a7f3d0", fontSize: 13, fontWeight: "700" }, toastMessage: { color: adminColors.white, marginTop: 5, fontSize: 16, fontWeight: "800" },
});
