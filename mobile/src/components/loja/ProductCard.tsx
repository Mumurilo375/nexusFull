...
function ProductCard({ game, listings, isFavorite, pendingFavorite, onOpen, onToggleFavorite }: ProductCardProps) {
  ...
  return (
    <View style={styles.card}>
      <Pressable accessibilityRole="button" accessibilityLabel={`Abrir detalhes de ${game.title}`} onPress={() => onOpen(game.id)} style={({ pressed }) => [styles.coverButton, pressed && styles.pressed]}>
        {imageUrl && !imageFailed ? <Image source={{ uri: imageUrl }} onError={() => setImageFailed(true)} resizeMethod="resize" fadeDuration={0} style={styles.cover} resizeMode="cover" /> : <View style={styles.imageFallback}><Ionicons name="image-outline" size={28} color="#64748b" /><Text style={styles.imageFallbackText}>Capa indisponível</Text></View>}
